...
export const CustomerReceipt = forwardRef<HTMLDivElement, { data: CustomerReceiptData }>(
  ({ data }, ref) => {
    ...
    return (
      ...
      >
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px", borderBottom: "3px solid #3b6cf5", paddingBottom: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            <div>
              ...
              {company.address && (
                <div style={{ fontSize: "12px", color: "#666", marginTop: "4px" }}>{company.address}</div>
              )}
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: "28px", fontWeight: 700, color: "#3b6cf5", fontFamily: "'JetBrains Mono', monospace" }}>
              INVOICE
            </div>
            <div style={{ fontSize: "16px", fontWeight: 600, fontFamily: "'JetBrains Mono', monospace", marginTop: "4px" }}>
              #{data.invoice_code}
            </div>
            <div style={{ fontSize: "12px", color: "#777", marginTop: "4px" }}>{date}</div>
            <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "24px" }}>
          <div style={{ background: "#f8f9fa", borderRadius: "8px", padding: "16px 20px", flex: "1", maxWidth: "45%" }}>
            <div style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1px", color: "#999", marginBottom: "8px" }}>Bill To</div>
            <div style={{ fontWeight: 600, fontSize: "16px" }}>{data.customer_name}</div>
            <div style={{ color: "#555", marginTop: "4px" }}>{data.customer_phone}</div>
            {data.customer_address && <div style={{ color: "#555", marginTop: "2px" }}>{data.customer_address}</div>}
          </div>
          {data.invoice_url && (
            <div style={{ textAlign: "center" }}>
              <QRCodeSVG value={data.invoice_url} size={90} level="M" />
              <div style={{ fontSize: "9px", color: "#999", marginTop: "4px" }}>Scan for digital copy</div>
            </div>
          )}
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "20px" }}>
          <thead>
            <tr style={{ background: "#3b6cf5", color: "#fff" }}>
          ...
          <tbody>
            {items.map((item, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #eee", background: i % 2 === 0 ? "#fafbfc" : "#fff" }}>
                <td style={{ padding: "10px 12px", fontSize: "12px", color: "#999" }}>{i + 1}</td>
                <td style={{ padding: "10px 12px", fontWeight: 500 }}>{item.product_name}</td>
                <td style={{ padding: "10px 12px", fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", color: "#777" }}>{item.product_code}</td>
                <td style={{ textAlign: "center", padding: "10px 12px" }}>{item.quantity}</td>
                <td style={{ textAlign: "right", padding: "10px 12px" }}>{Number(item.unit_price).toLocaleString()}</td>
                <td style={{ textAlign: "right", padding: "10px 12px", fontWeight: 600 }}>{Number(item.subtotal).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "20px" }}>
          <div style={{ width: "280px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", fontWeight: 700, fontSize: "16px", borderTop: "2px solid #3b6cf5", marginTop: "6px" }}>
              <span>Total Due</span>
              <span>{Number(data.total_due).toLocaleString()}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", color: "#3b6cf5", fontWeight: 600 }}>
              <span>COD</span>
        ...
        {/* Note */}
        {data.note && (
          <div style={{ padding: "12px 16px", background: "#fffde7", borderRadius: "6px", fontSize: "13px", color: "#555", marginBottom: "20px" }}>
            <strong>Note:</strong> {data.note}
          </div>
        )}

        {/* Footer */}
        <div style={{ textAlign: "center", paddingTop: "16px", borderTop: "2px dashed #ddd", fontSize: "12px", color: "#999" }}>
          <div style={{ fontWeight: 600, color: "#555" }}>Thank you for your business!</div>
